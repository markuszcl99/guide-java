...
public class ReentrantLockDemo {
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread thread1 = new Thread(new SafeIncrementByReentrantLockTask(sharedMemory));
        Thread thread2 = new Thread(new SafeIncrementByReentrantLockTask(sharedMemory));
        // 开启两个工作线程
        thread1.start();
        thread2.start();
        // 等待两个工作线程将任务执行完成
